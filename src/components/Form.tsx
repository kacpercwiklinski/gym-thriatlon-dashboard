...
const Form = ({ formInputs, onSubmit, onInputChange }: Props) => {
    ...
    const renderElement = (formInput: FormInput) => {
        if (formInput.type === "HTMLInputElement") {
            return <input name={formInput.name} onChange={onInputChange} type={formInput.valueType} placeholder={formInput.placeholder} value={formInput.value} className="input w-full max-w-[12rem] input-bordered" />
